...
__global__ void kernelCalcVertexInteraction(const double* rad, const double* pos, double* force, double* energy) {
  long vertexId = blockIdx.x * blockDim.x + threadIdx.x;
  if (vertexId < d_numVertices) {
    double thisRad, otherRad, radSum, interaction = 0.;
    ...
    for (long nListId = 0; nListId < d_maxNeighborListPtr[vertexId]; nListId++) {
      if(extractNeighbor(vertexId, nListId, pos, rad, otherPos, otherRad)) {
	  		...
	  		interaction += calcContactInteraction(thisPos, otherPos, radSum, &force[vertexId*d_nDim]);
...
inline __device__ double calcVertexVertexInteraction(const double* thisPos, const double* otherPos, const double radSum, double* currentForce, double sign) {
  double overlap, gradMultiple, distance;//, sign = 1;
	overlap = calcOverlap(thisPos, otherPos, radSum);
	if (overlap > 0) {
		//if(overlap > 0.5) {
		//	sign = -1;
		//}
		gradMultiple = d_ec * sign * overlap / radSum;
		distance = calcDistance(thisPos, otherPos);
	  for (long dim = 0; dim < d_nDim; dim++) {
	    currentForce[dim] += gradMultiple * pbcDistance(thisPos[dim], otherPos[dim], dim) / distance;
	  }
...
// this and other are for vertices belonging to neighbor particles
__global__ void kernelCalcVertexSegmentInteraction(const double* rad, const double* pos, double* force, double* pEnergy) {
  long vertexId = blockIdx.x * blockDim.x + threadIdx.x;
	long particleId = d_particleIdListPtr[vertexId];
  if (vertexId < d_numVertices) {
...
// this and other are for vertices belonging to neighbor particles
__global__ void kernelCalcVertexForceAngAcc(const double* rad, const double* pos, const double* pPos, double* force, double* torque, double* energy) {
  long vertexId = blockIdx.x * blockDim.x + threadIdx.x;
  if (vertexId < d_numVertices) {
		long particleId = d_particleIdListPtr[vertexId];
    double thisRad, otherRad, radSum, distance, overlap, gradMultiple;
    ...
    for (long nListId = 0; nListId < d_maxNeighborListPtr[vertexId]; nListId++) {
      if(extractNeighbor(vertexId, nListId, pos, rad, otherPos, otherRad)) {
				...
				if (overlap > 0) {
					...
					distance = calcDistance(thisPos, otherPos);
				  for (long dim = 0; dim < d_nDim; dim++) {
				    force[vertexId * d_nDim + dim] += gradMultiple * pbcDistance(thisPos[dim], otherPos[dim], dim) / distance;
				  }
				  energy[vertexId] += (0.5 * d_ec * overlap * overlap) * 0.5;
				}
			}
		}
		getParticlePos(particleId, pPos, partPos);
		getRelativeVertexPos(vertexId, pos, thisPos, partPos);
		torque[vertexId] = (thisPos[0] * force[vertexId * d_nDim + 1] - thisPos[1] * force[vertexId * d_nDim]) / calcNormSq(thisPos);
  }
}

__global__ void kernelCalcParticleForceAngAcc(const double* force, const double* torque, const double* energy, double* pForce, double* pTorque, double* pEnergy) {
	long particleId = blockIdx.x * blockDim.x + threadIdx.x;
	if(particleId < d_numParticles) {
		for (long dim = 0; dim < d_nDim; dim++) {
			pForce[particleId * d_nDim + dim] = 0;
...
__global__ void kernelCalcParticleForceTorque(const double* force, const double* torque, double* pForce, double* pTorque) {
	long particleId = blockIdx.x * blockDim.x + threadIdx.x;
	if(particleId < d_numParticles) {
		for (long dim = 0; dim < d_nDim; dim++) {
			pForce[particleId * d_nDim + dim] = 0;
...
__global__ void kernelTranslateVertices(const double* pDelta, double* pos) {
	long particleId = blockIdx.x * blockDim.x + threadIdx.x;
	if (particleId < d_numParticles) {
		long firstVertex = d_firstVertexInParticleIdPtr[particleId];
		long lastVertex = firstVertex + d_numVertexInParticleListPtr[particleId];
		for (long vertexId = firstVertex; vertexId < lastVertex; vertexId++) {
			for (long dim = 0; dim < d_nDim; dim++) {
				pos[vertexId * d_nDim + dim] += pDelta[particleId * d_nDim + dim];
				// pbc check
				//pos[vertexId * d_nDim + dim] -= floor(pos[vertexId * d_nDim + dim] / d_boxSizePtr[dim]) * d_boxSizePtr[dim];
			}
		}
	}
}

__global__ void kernelRotateVertices(const double* pDeltaAngle, const double* pPos, double* pos) {
	long particleId = blockIdx.x * blockDim.x + threadIdx.x;
	if (particleId < d_numParticles) {
		double thisPos[MAXDIM], newPos[MAXDIM];
		long firstVertex = d_firstVertexInParticleIdPtr[particleId];
		long lastVertex = firstVertex + d_numVertexInParticleListPtr[particleId];
		for (long vertexId = firstVertex; vertexId < lastVertex; vertexId++) {
			getDelta(&pos[vertexId * d_nDim], &pPos[particleId * d_nDim], thisPos);
			newPos[0] = thisPos[0] * cos(pDeltaAngle[particleId]) - thisPos[1] * sin(pDeltaAngle[particleId]);
			newPos[1] = thisPos[0] * sin(pDeltaAngle[particleId]) + thisPos[1] * cos(pDeltaAngle[particleId]);
			for (long dim = 0; dim < d_nDim; dim++) {
				pos[vertexId * d_nDim + dim] += (newPos[dim] - thisPos[dim]);
			}
		}
	}
}
...
__global__ void kernelCalcVertexDistanceSq(const double* pos, const double* initialPos, double* delta) {
	...
	if (vertexId < d_numVertices) {
		double distance;
		for (long dim = 0; dim < d_nDim; dim++) {
			distance = pbcDistance(pos[vertexId * d_nDim + dim], initialPos[vertexId * d_nDim + dim], d_boxSizePtr[dim]);
			delta[vertexId * d_nDim + dim] = distance * distance;
		}
	}
}
...
__global__ void kernelCalcVertexDisplacement(const double* pos, const double* lastPos, double* disp) {
	...
	if (vertexId < d_numVertices) {
		...
		for (long dim = 0; dim < d_nDim; dim++) {
			//delta = pbcDistance(pos[vertexId * d_nDim + dim], lastPos[vertexId * d_nDim + dim], d_boxSizePtr[dim]);
			delta = pos[vertexId * d_nDim + dim] - lastPos[vertexId * d_nDim + dim];
			distance += delta * delta;
		}
		disp[vertexId] = sqrt(distance);
	}
}
...
__global__ void kernelCalcVertexScatteringFunction(const double* pos, const double* initialPos, double* vSF, const double waveNum) {
	...
	if (vertexId < d_numVertices) {
		...
		delta = sqrt(deltaSq);
		vSF[vertexId] = sin(waveNum * delta) / (waveNum * delta);
	}
}
